      ...
      <div class="flex flex-row gap-4 pt-5">
        ...
        <div class="w-full">
          ...
          >
            ...
            <FuelUseRegisterFields v-if="selectedGroup && ['FUEL'].includes(selectedGroup.form_name!)" />
            <OrganizationVehicleRegisterFields v-else-if="selectedGroup && ['ORGANIZATION_VEHICLES'].includes(selectedGroup.form_name!)" />
            <ElectricityRegisterFields v-else-if="selectedGroup && ['ELECTRICITY'].includes(selectedGroup.form_name!)" />
            <RefrigerantRegisterFields v-else-if="selectedGroup && ['REFRIGERANTS'].includes(selectedGroup.form_name!)" />
            <WasteRegisterFields v-else-if="selectedGroup && ['WASTE'].includes(selectedGroup.form_name!)" />
            <ServiceRegisterFields v-else-if="selectedGroup && ['SERVICES'].includes(selectedGroup.form_name!)" />
            <LeasedAssetRegisterFields v-else-if="selectedGroup && ['LEASED_ASSETS'].includes(selectedGroup.form_name!)" />
            <TransportationRegisterFields v-else-if="selectedGroup && ['TRANSPORTATION'].includes(selectedGroup.form_name!)" />
            <ProductRegisterFields v-else-if="selectedGroup && ['PRODUCTS'].includes(selectedGroup.form_name!)" />
            <InvestmentRegisterFields v-else-if="selectedGroup && ['INVESTMENTS'].includes(selectedGroup.form_name!)" />
            <!-- Attachment Section -->
            <div class="mb-5">
              <ul>
